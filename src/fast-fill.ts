...
(() => {
	...
	const DEFAULT_PLACEHOLDERS: Record<string, string> = Object.freeze({
		ser: 'search "{{_}}"'
	});

	// Map avoids prototype-pollution keys (__proto__, constructor) entirely.
	const placeholders: Map<string, string> = new Map(Object.entries(DEFAULT_PLACEHOLDERS));
	...
	// Hand-written minimal types (compile-time only, no runtime output).
	type ResolveMode = 'clipboard' | 'empty';
	...
	const hasPlaceholder = (word: unknown): boolean => {
		return typeof word === 'string' && word.length > 0 && placeholders.has(word);
	};

	const getWordBeforeCursor = (text: unknown): string => {
	...
	const dispatchInputEvent = (target: EventTarget, data: string): void => {
	...
	const hasUnescapedSlot = (template: unknown): boolean => {
	 ...
	 */
	const resolveTemplate = (template: string, { mode, clip }: ResolveOptions): ResolveResult => {
	...
	const peekMonacoTemplate = (activeElement: Element): string | null => {
		...
		try {
			editor = monaco.editor!.getEditors!().find(
				(ed: MonacoEditor) => typeof ed.getDomNode === 'function' && ed.getDomNode() === editorNode
			);
		} catch {
			return null;
		}
	...
	const peekPlainTemplate = (activeElement: Element): string | null => {
		if (activeElement.tagName !== 'TEXTAREA' && activeElement.tagName !== 'INPUT') {
			return null;
		}
		if (
			activeElement.tagName === 'INPUT' &&
			!SUPPORTED_INPUT_TYPES.has((activeElement as HTMLInputElement).type)
		) {
			return null;
		}
		if (
		...
		if (typeof (activeElement as HTMLInputElement).value !== 'string') {
			return null;
		}
		if (
			typeof (activeElement as HTMLInputElement).selectionStart !== 'number' ||
			(activeElement as HTMLInputElement).selectionStart !==
				(activeElement as HTMLInputElement).selectionEnd
		) {
			return null;
		}
		const cursorPos = (activeElement as HTMLInputElement).selectionStart as number;
		const word = getWordBeforeCursor((activeElement as HTMLInputElement).value.slice(0, cursorPos));
		if (!hasPlaceholder(word)) {
			return null;
		}
		return placeholders.get(word) ?? null;
	...
	const peekContentEditableTemplate = (activeElement: Element): string | null => {
	...
	const peekTemplate = (activeElement: Element): string | null => {
		return (
			peekMonacoTemplate(activeElement) ??
	...
	const handleMonaco = async (
		...
		try {
			...
			const range = new (monaco.Range as MonacoRangeCtor)(
				position.lineNumber,
				startColumn,
				position.lineNumber,
				position.column
	...
	const handlePlainField = async (
		...
		try {
			...
			(activeElement as HTMLTextAreaElement | HTMLInputElement).setRangeText(
				text,
				start,
				cursorPos,
				'end'
			...
			if (typeof caretOffset === 'number') {
				...
				).selectionEnd = start + caretOffset;
			}
		} catch {
			return false;
		}
	...
	const handleContentEditable = async (
		activeElement: Element,
		text: string,
		caretOffset: number | null
	): Promise<boolean> => {
